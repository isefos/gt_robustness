...
class Attack(ABC):
    ...
    def __init__(self,
                 adj: Union[SparseTensor, torch.Tensor],  # TensorType["n_nodes", "n_nodes"]],
                 attr: torch.Tensor,  # TensorType["n_nodes", "n_features"],
                 labels: torch.Tensor,  # TensorType["n_nodes"],
                 idx_attack: np.ndarray,
                 model,
                 device: Union[str, int, torch.device],
                 data_device: Union[str, int, torch.device],
    ...
    def attack(self, n_perturbations: int, **kwargs):
        ...
        if n_perturbations > 0:
            ...
            self.attr_adversary = self.attr
            self.adj_adversary = self.adj
        # return pert_edge_index and pert_edges
...
class SparseAttack(Attack):
    ...
    def __init__(self,
                 adj: Union[SparseTensor, torch.Tensor],  # TensorType["n_nodes", "n_nodes"], sp.csr_matrix],
                 **kwargs):
...
class SparseLocalAttack(SparseAttack):
    ...
    def get_logits(self,
                   ...
                   perturbed_graph: Optional[Union[SparseTensor,
                                                   Tuple[torch.Tensor,  # TensorType[2, "nnz"],
                                                         torch.Tensor]]] = None,  # TensorType["nnz"]]]] = None
                  ):
        pass
    ...
    @staticmethod
    def classification_statistics(logits: torch.Tensor,  # TensorType[1, "n_classes"],
                                  label: torch.Tensor,  # TensorType[()]
                                  ) -> Dict[str, float]:
